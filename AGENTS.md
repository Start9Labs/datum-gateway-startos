...
- **bitcoind is reached over the LXC bridge**, not `.startos` DNS. `bitcoindRpcUrl` in `startos/utils.ts` resolves bitcoind's RPC URL from Knots' exported `rpcHostId`/`rpcInterfaceId` (imported from `bitcoin-knots-startos/startos/utils`); `main.ts` pins it into the config file's `rpcurl` each run. The file model types `rpcurl` loosely (`z.string()` with a legacy `bitcoind.startos` catch) because the address is dynamic.
